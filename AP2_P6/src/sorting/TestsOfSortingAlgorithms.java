 ...
 */
public final class TestsOfSortingAlgorithms {
    private TestsOfSortingAlgorithms() {}
    ...
    private static double performanceMeasurement(Algorithm algo, int[] array) {
        ...
        if (repetitions == 1)
            return time;
        //for (int i = 0; i < 10; i++) repeatedSorting(algo, array, repetitions);
    ...
    private static double firstCheck(Algorithm algo, int[] array) {
        ...
        if (! isCorrect) throw new IllegalStateException(); 
    ...
    private static double repeatedSorting(Algorithm algo, int[] array,
        ...
        for (int i = 0; i < repetitions; i++) {
            int[] testArray = array.clone();
        	//int[] testArray = randomArray(array.length);
            long startTime = System.nanoTime();
            algo.sort(testArray);
            long endTime = System.nanoTime();
